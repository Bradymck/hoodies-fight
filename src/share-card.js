// KO share-card generator - a self-contained frontend module. Called from
// main.js's showMatchOverActions once a match is decided (see the Share
// button wiring there), using the same live match canvas/winner/loser/
// wins data that screen already has at hand.
...
// Same 20:9 ratio as the real match canvas (#canvas in index.html, 800x360) -
...
// the source canvas actually renders at (its CSS size may be scaled up by
// the wrapper's own fit-to-screen sizing in main.js - drawImage reads a
// canvas source by its own pixel buffer regardless, so that scale is
// transparent to this file too).
...
// busy/bright arena screenshot - a glow alone nearly vanishes against a
// light background. Canvas has no paint-order property, so the same effect
// is just "stroke first, fill on top" in plain draw-call order, which is
// paint-order's default anyway.
...
// updateRoundInfo), OR a plain pre-formatted string, so the caller can hand
// this either the raw wins tally or its own formatted label without this
// file dictating which.
...
export async function renderKOShareCard({ winnerName, loserName, roundScore, winnerCanvas } = {}) {
  ...
  if (winnerCanvas && winnerCanvas.width > 0 && winnerCanvas.height > 0) {
    ...
    // main.js's own fit-to-screen wrapper sizing upscaling the CSS display
    // size past the logical 800x360 the canvas element's own width/height
  ...
  // screenshot rather than computed from font metrics alone.
